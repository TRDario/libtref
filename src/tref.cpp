...
tref::OutputBitmap::OutputBitmap(std::byte *data, unsigned int width,
                                 unsigned int height) noexcept
    : _data{data}, _width{width}, _height{height} {}

tref::OutputBitmap::~OutputBitmap() noexcept { std::free(_data); }

std::span<const std::byte> tref::OutputBitmap::data() const noexcept {
  return {_data, _width * _height * 4};
}

unsigned int tref::OutputBitmap::width() const noexcept { return _width; }

unsigned int tref::OutputBitmap::height() const noexcept { return _height; }
...
tref::DecodingResult tref::decode(std::istream &is) {
  ...
  std::copy(std::istreambuf_iterator<char>{is},
            ...
            std::back_inserter(compressedData));

  std::vector<char> uncompressedData(uncompressedSize);
  const auto reportedUncompressedSize{
      LZ4_decompress_safe(compressedData.data(), uncompressedData.data(),
  ...
  qoi_desc desc;
  auto decodedImage{
      qoi_decode(ptr, uncompressedData.size() - (ptr - uncompressedData.data()),
                 &desc, 4)};
  if (decodedImage == nullptr) {
    throw DecodingError{"Failed to decode QOI data."};
  }
  return {lineSkip, std::move(glyphs),
          OutputBitmap{(std::byte *)(decodedImage), desc.width, desc.height}};
}

void tref::encode(std::ostream &os, std::int32_t lineSkip, GlyphMap glyphs,
                  InputBitmap bitmap) {
  ...
  buffer.resize(LZ4_compress_default(data.c_str(), buffer.data(), data.size(),
                                     buffer.size()));
  os.write("TREF", 4);
